require_relative "./draught_house_tap/tap_list_scraper.rb"
require_relative "./draught_house_tap/beer.rb"

class DraughtHouseTap
  def run
    puts "Hi, below is a list of current beers on tap at Draught House. "
    beers =TapListScraper.scrape_index_page
    beers.each do |beer|
      Beer.new(beer)
    ...
    while !quit
      ...
      if beer != nil
        TapListScraper.scrape_profile_page(beer)
